...
int main(){
	// Matris A ve B ye rastgele int değişkenler atıp doldurduktan sonra MAtrisC yi yani çarpım sonucu olan matrisi 0 lıyor aynı zamanda
	matrisleriDoldur(1000);

	cout<<"****\nINT Degiskenler çarpılıyor...\nHer Satir için ayrı Thread oluşturuldu.."<<endl;
...
void paralelCarpim(int satirSayisi){
	...
	pthread_t threads[(satirSayisi)];  // satır sayısı kadar thread belirle
	for (int i = 0; i < satirSayisi; i++){			//Elde edilen her matris için bir thread oluşturuyoruz
